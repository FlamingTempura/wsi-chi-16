...
The term \emph{evidence-based medicine} which refers to the use of epidemiological methods in both patient-level decision making and formulating population-level clinical care guidelines.  In the first use of the term, David M. Eddy pointed out that while outcome analysis may make it seem that there is a clear path towards achiving more accurate, or at least consistent diagnoses, things are never that simple in practice~\cite{eddy1990challenge}:
...
We started with a set of search terms broad enough to encompass studies of clinical practice where patient-logged data (both paper-based and digital, manual and automatic) were introduced into a clinical setting.  To do this we searched PubMed for keywords ``patient diaries'', ``self-report diaries'', ``quantified-self'', ``smartphone apps'', ``wearable sensors'', and ``telemonitoring''. 

Since we wanted to focus on the usage of data by medical experts, we excluded studies about use by patients themselves, such as for feedback, reflection, goal setting and self monitoring.   Focusing only on existing practice, we omitted papers describing new interfaces and systems that have not had substantial adoption, as well as papers focused on the capture side of health diarising and life-logging by patients, except where aspects of capture affected its later use.  Since, however factors pertaining to the use of patient-related data in general may be particularly salient or problematic for the QS logged case, we included factors that might influence all use of patient data, including human factors issues of data interfaces, electronic medical records, and situational constraints of clinical settings, including operating theatres and emergency rooms.

After finding a small set of results, we then broadened our search to include studies that discussed the use of patient data in medical decision-making, including both patient-supplied and clinical data held by providers themselves.  We hoped to find a broad range of factors spanning human-factors issues to social, cultural, institutional, situational, among others.


\subsection{Results: High Level}

\subsection{Results: Cognitive Dispositions to Respond}

During the first phase, we encountered a substantial body of work pertaining to  situational and psychological factors that influenced the decision-making performance of medical experts in clinical settings.  Croskerry's \emph{Cognitive Dispositions to Respond}~\cite{} we discovered a large body of work pertaining to 
