...
From a clinical perspective, what are the problems

\begin{enumerate}
    \item \emph{Data subject}: \emph{What} is captured?
    \item \emph{Sampling}: \emph{How} and \emph{when} information is captured?
    \item \emph{Accuracy}: Trusting the \emph{veracity/quality} of the instruments or capture method?
    \item \emph{Access}: Ease of \emph{access} to the data?
    \item \emph{Interactional}: Interfere with patient visits?
    \item \emph{Workflow}:Workflow not accommodate use
    \item \emph{Biases}: Cognitive biases introduced by looking at data
